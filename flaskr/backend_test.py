...
def test_sign_up(storage_client,blob,bucket,user_name,user_password,user_first_name,user_last_name):
    b = Backend(storage_client)
...
def test_sign_in_success(storage_client,blob,bucket,user_name,user_password,user_info_json_loads,user_info):
    with patch('google.cloud.storage.Blob') as storage_blob:
...
def test_get_user_info(storage_client,blob,bucket,user_name,user_password,user_info_json_loads,user_info):
    blob.download_as_text.return_value = user_info_json_loads
    json_output = json.loads(blob.download_as_text())
    b = Backend(storage_client)
